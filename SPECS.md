...
|------|----------------|--------|
| [SPEC_meiosis_atlas_pages.md](specs_todo/SPEC_meiosis_atlas_pages.md) | Master page-set overview: the 5 hub pages + 3 missing builders + cross-atlas dependencies | nco shipped (see above); others pending |
| [SPEC_crossovers_page.md](specs_todo/SPEC_crossovers_page.md) | `crossovers` page: CO + DCO cohort view (per-dyad/chrom counts, rate/Mb, refined breakpoint track, karyotype-stratified intrachromosomal-effect view) | adapter pair for `chromosome_meiosis_events_v1` not yet built |
| [SPEC_interchromosomal_page.md](specs_todo/SPEC_interchromosomal_page.md) | `interchromosomal` page (HEADLINE): does het at focal inversion alter meiosis on OTHER chromosomes? Family-aware permutation + local-inversion controls. | blocked by 4 missing builders; one of which is the `chromosome_meiosis_events` adapter |
