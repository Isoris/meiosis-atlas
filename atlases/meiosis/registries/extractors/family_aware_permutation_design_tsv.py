"""Meiosis-atlas staging extractor — family_aware_permutation_design TSV →
staging rows.

Twin of `tract_classifications_tsv.py`,
`chromosome_meiosis_events_tsv.py`, and `local_inv_controls_tsv.py`.
Parses any tab- (or whitespace-) separated file into {columns, rows}
and tags it with optional scope / source metadata. Type coercion +
karyotype-enum validation happen in the
normalize_family_aware_permutation_design extractor downstream.
"""
