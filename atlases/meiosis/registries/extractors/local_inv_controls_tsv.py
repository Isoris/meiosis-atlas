"""Meiosis-atlas staging extractor — local_inv_controls TSV → staging rows.

Twin of `tract_classifications_tsv.py` and `chromosome_meiosis_events_tsv.py`.
Parses any tab- (or whitespace-) separated file into {columns, rows} and
tags it with optional scope / source metadata. Type coercion happens in
the normalize_local_inv_controls extractor downstream.
"""
