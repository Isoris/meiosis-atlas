"""Meiosis-atlas normalizer — promotes staging_local_inv_controls_v0 →
local_inv_controls_v1.

Twin of `normalize_tract_classifications.py` and
`normalize_chromosome_meiosis_events.py`. Pure file-IO: reads the
workspace layers index to find the source envelope path, copies its
JSON into raw_results/ for provenance, hands the path to the extractor.
